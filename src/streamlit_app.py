...
def initialize_session_state():
    ...
    return str(uuid.uuid4())  # Generate a unique thread ID for the session
...
# ---------------- Bot Interaction ----------------
def build_message_content(user_input, uploaded_image):
    """
    ...
    if uploaded_image:
        content.append(encode_uploaded_image(uploaded_image))
    return content


def generate_bot_response(thread_id, message_content):
    """
    ...
    conf = {"configurable": {"thread_id": thread_id}}  # Unique thread ID
    response = qa_graph.graph.invoke({"messages": messages}, conf)
    return response['messages'][-1].content, response['article_ids']


# ---------------- Core Message Processor ----------------
def process_user_message(thread_id, user_input, uploaded_image):
    """
    Handle full interaction flow: process user message, call model, and update UI.

    Args:
        thread_id (str): ID for the current conversation
        user_input (str): User's text input.
        uploaded_image (UploadedFile): Optional image uploaded with message.
    """
    message_content = build_message_content(user_input, uploaded_image)

    append_user_message(user_input, uploaded_image)

    ai_message, article_ids = generate_bot_response(thread_id, message_content)
    image_paths = get_article_image_paths(article_ids)

    append_assistant_message(ai_message, image_paths)
...
def main():
    ...
    thread_id = initialize_session_state()
    display_messages()

    user_input, uploaded_image, submitted = get_user_input()
    if submitted:
        process_user_message(thread_id, user_input, uploaded_image)
